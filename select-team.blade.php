    ...
    <div class="max-w-7xl mx-auto px-4 pb-8">
        <div class="mt-6 mb-6">
            ...
            <p class="mt-2 max-w-3xl text-sm md:text-base text-text-secondary">
                Construye tu proyecto en el futbol femenino: elige club o seleccion, define una identidad de juego y compite temporada a temporada.
            </p>
        </div>
        ...
        <div x-data="{
            ...
            <form method="post" action="{{ route('init-game') }}" @submit="loading = true" class="space-y-6">
                ...
                @if($hasTournamentMode)
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                        <button type="button"
                                @click="mode = 'career'"
                                :class="mode === 'career'
                                    ? 'ring-2 ring-accent-primary border-accent-primary/30 bg-accent-primary/5'
                            ...
                            <div class="shrink-0 w-12 h-12 md:w-14 md:h-14 rounded-xl flex items-center justify-center"
                                 :class="mode === 'career' ? 'bg-accent-primary' : 'bg-surface-600'">
                            ...
                            <div class="flex-1 min-w-0">
                                <h3 class="font-heading font-bold text-base md:text-lg uppercase tracking-wide" :class="mode === 'career' ? 'text-accent-primary' : 'text-text-body'">
                                    {{ __('game.mode_career') }}
                                </h3>
                                <p class="text-xs md:text-sm mt-0.5" :class="mode === 'career' ? 'text-accent-primary/80' : 'text-text-muted'">
                                    {{ __('game.mode_career_desc') }}
                                </p>
                            </div>
                            <div x-show="mode === 'career'" x-cloak class="shrink-0">
                                <svg class="w-6 h-6 text-accent-primary" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                ...
                <div class="rounded-xl border border-border-default bg-surface-800/80 px-4 py-3 text-sm text-text-secondary">
                    Esta primera version ya presenta la experiencia como manager de futbol femenino. La simulacion y la estructura de competiciones se mantienen mientras adaptamos datos y plantillas especificas.
                </div>
